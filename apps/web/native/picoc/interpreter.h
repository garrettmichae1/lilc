...
struct Picoc_Struct {
    ...
    /* FILE is incomplete on Emscripten. PicoC only uses CStdOut as a pointer. */
    char CStdOutBase[1];
#else
    IOFILE CStdOutBase;
#endif
...
#if defined(UNIX_HOST) || defined(WIN32) || defined(LILC_IOS_HOST)
...
/* ISO headers emulated in include.c / stdlib.c */
extern const char StddefDefs[];
extern const char StdintDefs[];
extern void StddefSetupFunc(Picoc *pc);
extern void LimitsSetupFunc(Picoc *pc);
