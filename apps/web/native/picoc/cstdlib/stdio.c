...
#ifdef LILC_IOS_HOST
#include "../lilc/lilc_picoc_runner.h"

extern void LilCOutputAppend(const char *Bytes, int Length);

static FILE *LilCInputFile(FILE *Stream)
{
    FILE *stdinFile = LilCStdinFile();
    if (Stream == NULL || Stream == stdin || Stream == stdinFile) {
        return stdinFile;
    }
    return Stream;
}

static int LilCIsCapturedFile(FILE *FilePtr)
{
    return FilePtr == stdout || FilePtr == stderr;
}

/*
 * PicoC stores every floating-point value as a C `double` (see union AnyValue).
 * A user program that calls scanf("%f", &x) therefore hands us a pointer to an
 * 8-byte double, but the "%f" conversion tells the host libc to store a 4-byte
 * float, which corrupts the value. To make scanf behave like a normal C
 * terminal we rewrite every floating conversion so the host writes a double:
 * %f/%e/%g/%a (and their width/precision variants) become %lf/%le/%lg/%la, and
 * any existing length modifier (h, l, ll, L, j, z, t) on a float conversion is
 * normalized to a single `l`. Integer and string conversions are left intact
 * because PicoC's int/char storage already matches the host.
 *
 * Caller owns the returned buffer and must free() it.
 */
static char *LilCNormalizeScanfFormat(const char *Format)
{
    size_t SrcLen = strlen(Format);
    /* Worst case: each conversion grows by one 'l'; allocate generously. */
    ...
    while (i < SrcLen) {
        ...
        /* Literal "%%" - copy the second '%' and move on. */
        if (i < SrcLen && Format[i] == '%') {
            Out[OutPos++] = Format[i++];
            continue;
        }

        /* Copy flags, field width, '.', precision, and assignment suppression
           (everything up to the length modifier / conversion specifier), but
           remember where any length modifier begins so we can rewrite it. */
        while (i < SrcLen) {
            char C = Format[i];
            if ((C >= '0' && C <= '9') || C == '*' || C == '.' ||
                C == '\'' || C == '-' || C == '+' || C == ' ' || C == '#') {
        ...
        /* Capture (and skip) any existing length modifier. */
        char LengthMod[3] = {0, 0, 0};
        size_t LenModCount = 0;
        while (i < SrcLen && LenModCount < 2) {
            char C = Format[i];
            if (C == 'h' || C == 'l' || C == 'L' || C == 'j' ||
                C == 'z' || C == 't' || C == 'q') {
                LengthMod[LenModCount++] = C;
                i++;
            } else {
                break;
            }
        ...
        if (i >= SrcLen) {
            /* Trailing malformed conversion - emit captured modifier verbatim. */
            for (size_t m = 0; m < LenModCount; m++)
                Out[OutPos++] = LengthMod[m];
            break;
        }

        char Conv = Format[i++];
        int IsFloatConv = (Conv == 'f' || Conv == 'F' || Conv == 'e' ||
                           Conv == 'E' || Conv == 'g' || Conv == 'G' ||
                           Conv == 'a' || Conv == 'A');

        if (IsFloatConv) {
            /* Force a double-sized store to match PicoC's storage. */
            Out[OutPos++] = 'l';
        } else {
            /* Preserve the caller's length modifier for non-float conversions. */
            for (size_t m = 0; m < LenModCount; m++)
                Out[OutPos++] = LengthMod[m];
        }
        Out[OutPos++] = Conv;
...
static int LilCAppendFormat(const char *Format, ...)
{
    va_list Args;
    va_list Copy;
    int Length;
    ...
    va_copy(Copy, Args);
    Length = vsnprintf(StackBuffer, sizeof(StackBuffer), Format, Args);
    va_end(Args);
    ...
    if ((size_t)Length < sizeof(StackBuffer)) {
        LilCOutputAppend(StackBuffer, Length);
        va_end(Copy);
        return Length;
    }

    HeapBuffer = malloc((size_t)Length + 1);
    if (HeapBuffer == NULL) {
        va_end(Copy);
        return 0;
    }

    vsnprintf(HeapBuffer, (size_t)Length + 1, Format, Copy);
    va_end(Copy);
    LilCOutputAppend(HeapBuffer, Length);
    free(HeapBuffer);
    return Length;
}
#endif
...
{
#ifdef LILC_IOS_HOST
    if (Stream->FilePtr != NULL && LilCIsCapturedFile(Stream->FilePtr)) {
        char Byte = (char)OutCh;
        LilCOutputAppend(&Byte, 1);
        Stream->CharCount++;
        return;
    }
#endif
...
void StdioOutPuts(const char *Str, StdOutStream *Stream)
{
    ...
    if (Stream->FilePtr != NULL && LilCIsCapturedFile(Stream->FilePtr)) {
        ...
        LilCOutputAppend(Str, Length);
        Stream->CharCount += Length;
        return;
    }
#endif
    if (Stream->FilePtr != NULL) {
...
#ifdef LILC_IOS_HOST
    if (Stream->FilePtr != NULL && LilCIsCapturedFile(Stream->FilePtr)) {
        Stream->CharCount += LilCAppendFormat(Format, Value);
        return;
    }
#endif
    if (Stream->FilePtr != NULL)
...
void StdioFprintfLong(StdOutStream *Stream, const char *Format, uint64_t Value) {
    ...
    if (Stream->FilePtr != NULL) {
#ifdef LILC_IOS_HOST
        if (LilCIsCapturedFile(Stream->FilePtr)) {
            Stream->CharCount += LilCAppendFormat(PlatformFormat, Value);
            return;
        }
#endif
...
#ifdef LILC_IOS_HOST
    {
        char *SafeFormat = LilCNormalizeScanfFormat(Format);
        const char *UseFormat = SafeFormat != NULL ? SafeFormat : Format;
        int Result;
        ...
        if (Stream != NULL) {
            ...
            Result = sscanf(StrIn, UseFormat, ScanfArg[0], ScanfArg[1],
                ...
                ScanfArg[6], ScanfArg[7], ScanfArg[8], ScanfArg[9]);
...
{
#ifdef LILC_IOS_HOST
    const char *Name = Param[0]->Val->Pointer;
    const char *Mode = Param[1]->Val->Pointer;
    if (Name == NULL || Mode == NULL) {
        ReturnValue->Val->Pointer = NULL;
        return;
    }
    if (!LilCProjectPathIsSafe(Name)) {
        ProgramFail(Parser, "cannot open '%s' outside this project folder", Name);
    }
    ReturnValue->Val->Pointer = LilCFopenProjectFile(Name, Mode);
#else
    ...
    struct Value **Param, int NumArgs)
{
    ...
    const char *Name = Param[0]->Val->Pointer;
    const char *Mode = Param[1]->Val->Pointer;
    if (Name == NULL || Mode == NULL || Param[2]->Val->Pointer == NULL) {
        ReturnValue->Val->Pointer = NULL;
        return;
    }
    if (!LilCProjectPathIsSafe(Name)) {
    ...
    {
        char FullPath[2048];
        if (LilCResolveProjectPath(Name, FullPath, sizeof(FullPath)) != 0) {
            ReturnValue->Val->Pointer = NULL;
            return;
        }
        ReturnValue->Val->Pointer = freopen(FullPath, Mode, Param[2]->Val->Pointer);
    }
#else
    ...
    struct Value **Param, int NumArgs)
{
    if (Param[0]->Val->Pointer == NULL)
        ProgramFail(Parser, "NULL pointer passed to fclose()");
    ...
    struct Value **Param, int NumArgs)
{
    ...
    ReturnValue->Val->Integer = fread(Param[0]->Val->Pointer,
        ...
        LilCInputFile(Param[3]->Val->Pointer));
#else
    ReturnValue->Val->Integer = fread(Param[0]->Val->Pointer,
    ...
    struct Value **Param, int NumArgs)
{
    ...
    const char *Name = Param[0]->Val->Pointer;
    char FullPath[2048];
    if (Name == NULL || !LilCProjectPathIsSafe(Name)) {
        ProgramFail(Parser, "cannot open '%s' outside this project folder",
            Name != NULL ? Name : "");
    }
    if (LilCResolveProjectPath(Name, FullPath, sizeof(FullPath)) != 0) {
    ...
    ReturnValue->Val->Integer = remove(FullPath);
#else
...
{
#ifdef LILC_IOS_HOST
    const char *From = Param[0]->Val->Pointer;
    const char *To = Param[1]->Val->Pointer;
    char FromPath[2048];
    char ToPath[2048];
    if (From == NULL || To == NULL || !LilCProjectPathIsSafe(From) ||
            !LilCProjectPathIsSafe(To)) {
        ProgramFail(Parser, "cannot open '%s' outside this project folder",
            From != NULL ? From : "");
    }
    if (LilCResolveProjectPath(From, FromPath, sizeof(FromPath)) != 0 ||
            LilCResolveProjectPath(To, ToPath, sizeof(ToPath)) != 0) {
        ReturnValue->Val->Integer = -1;
        return;
    }
    ReturnValue->Val->Integer = rename(FromPath, ToPath);
#else
...
{
#ifdef LILC_IOS_HOST
    rewind(LilCInputFile(Param[0]->Val->Pointer));
#else
...
{
#ifdef LILC_IOS_HOST
    clearerr(LilCInputFile(Param[0]->Val->Pointer));
#else
...
{
#ifdef LILC_IOS_HOST
    ReturnValue->Val->Integer = feof(LilCInputFile(Param[0]->Val->Pointer));
#else
...
{
#ifdef LILC_IOS_HOST
    char Byte = (char)Param[0]->Val->Integer;
    LilCOutputAppend(&Byte, 1);
    ...
    struct Value **Param, int NumArgs)
{
    ...
    ReturnValue->Val->Integer = ungetc(Param[0]->Val->Integer,
        LilCInputFile(Param[1]->Val->Pointer));
#else
    ReturnValue->Val->Integer = ungetc(Param[0]->Val->Integer,
        Param[1]->Val->Pointer);
#endif
...
#ifdef LILC_IOS_HOST
    char Newline = '\n';
    const char *Text = Param[0]->Val->Pointer;
    int Length;
    if (Text == NULL)
        ProgramFail(Parser, "NULL pointer passed to puts()");
    Length = (int)strlen(Text);
    LilCOutputAppend(Text, Length);
    LilCOutputAppend(&Newline, 1);
    ReturnValue->Val->Integer = Length + 1;
    return;
#endif
...
#ifdef LILC_IOS_HOST
    ReturnValue->Val->Integer = StdioBaseScanf(Parser, LilCStdinFile(), NULL,
...
void StdioSetupFunc(Picoc *pc)
{
    ...
    /* Emscripten's FILE is an incomplete type (opaque pointer). */
    StructFileType = TypeCreateOpaqueStruct(pc, NULL,
        TableStrRegister(pc, "__FILEStruct"), 256);
#else
    StructFileType = TypeCreateOpaqueStruct(pc, NULL,
...
{
#ifdef LILC_IOS_HOST
    if (LilCIsCapturedFile(Stream)) {
        LilCOutputAppend(&OutCh, 1);
        return;
    }
#endif
    putc(OutCh, Stream);
...
void PrintStr(const char *Str, FILE *Stream)
{
    ...
    if (LilCIsCapturedFile(Stream)) {
        if (Str != NULL)
            LilCOutputAppend(Str, (int)strlen(Str));
        return;
    }
#endif
    fputs(Str, Stream);
...
void PrintFP(double Num, FILE *Stream)
{
    ...
    if (LilCIsCapturedFile(Stream)) {
        char buf[64];
        int n = snprintf(buf, sizeof(buf), "%f", Num);
        if (n > 0)
            LilCOutputAppend(buf, n);
        return;
    }
