    ...
    int *IsStatic)
{
    ...
    *Typ = NULL;
    ...
    /* ignore leading type qualifiers (const/volatile are accepted and ignored
       for semantics, but must be consumed so declarations like
       "const int x" or "const void *p" parse correctly) */
    ParserCopy(&Before, Parser);
    Token = LexGetToken(Parser, &LexerValue, true);
    while (Token == TokenStaticType || Token == TokenAutoType ||
            Token == TokenRegisterType || Token == TokenExternType ||
            Token == TokenConstType || Token == TokenVolatileType) {
    ...
    *Identifier = Parser->pc->StrEmpty;
    ...
    while (!Done) {
        ...
        case TokenVolatileType:
            /* qualifiers anywhere in the declarator (e.g. "int const x" or
               "char * const p") are accepted and ignored */
            if (*Typ == NULL)
                ProgramFail(Parser, "bad type declaration");
            break;
