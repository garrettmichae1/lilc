...
static enum LexToken LexGetCharacterConstant(Picoc *pc, struct LexState *Lexer,
    struct Value *Value);
static void LexSkipComment(Picoc *pc, struct LexState *Lexer, char NextChar);
    ...
    struct Value *Value, char EndChar)
{
    ...
    EndPos = Lexer->Pos;
    if (Lexer->Pos == Lexer->End)
        LexFail(pc, Lexer, "unterminated string");
...
void LexSkipComment(Picoc *pc, struct LexState *Lexer, char NextChar)
{
    if (NextChar == '*') {
        ...
        if (Lexer->Pos == Lexer->End)
            LexFail(pc, Lexer, "unterminated comment");
        ...
        *Value = &pc->LexValue;
        ...
        case '/':
            if (NextChar == '/' || NextChar == '*') {
                LEXER_INC(Lexer);
                LexSkipComment(pc, Lexer, NextChar);
            } else
