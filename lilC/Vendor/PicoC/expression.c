...
{
    if (t >= TokenIntType && t <= TokenVolatileType)
        return 1; /* base type (incl. const/volatile qualifiers) */
    ...
    struct Value *BottomValue, struct Value *TopValue)
{
    ...
    if (Op == TokenLeftSquareBracket) {
        ...
        case TypeArray:
            ...
            break;
        case TypePointer:
            if (BottomValue->Val->Pointer == NULL)
                ProgramFail(Parser, "NULL pointer dereference");
            Result = VariableAllocValueFromExistingData(Parser,
            BottomValue->Typ->FromType,
        ...
        case TokenDivideAssign:
            if (TopInt == 0)
                ProgramFail(Parser, "division by zero");
            ResultInt = ExpressionAssignInt(Parser, BottomValue,
                BottomInt/TopInt, false);
            break;
        case TokenModulusAssign:
            if (TopInt == 0)
                ProgramFail(Parser, "modulo by zero");
            ResultInt = ExpressionAssignInt(Parser, BottomValue,
                BottomInt%TopInt, false);
            break;
        case TokenShiftLeftAssign:
            if (TopInt < 0 || TopInt >= (long)(sizeof(long) * 8))
                ProgramFail(Parser, "invalid shift count");
            ResultInt = ExpressionAssignInt(Parser, BottomValue,
                BottomInt<<TopInt, false);
            break;
...
int ExpressionParse(struct ParseState *Parser, struct Value **Result)
{
    ...
    int TernaryDepth = 0;
    /* Tracks the boolean condition for each nested ?: so that only the taken
       branch is evaluated (C requires the untaken branch to be skipped, which
       also makes recursive/side-effecting ternaries behave correctly). */
    #define MAX_TERNARY_NESTING 32
    int TernaryCondition[MAX_TERNARY_NESTING];
    int TernaryIgnoreDepth = -1; /* which ternary level owns the current ignore */
    struct Value *LexValue;
    ...
    do {
        ...
        if ((((int)Token > TokenComma && (int)Token <= (int)TokenOpenBracket) ||
            ...
            if (PrefixState) {
                ...
                if (OperatorPrecedence[(int)Token].PostfixPrecedence != 0) {
                    ...
                    if (Token == TokenDot || Token == TokenArrow) {
                        ...
                        /* ternary '?': remember the condition. When it's
                            false we must not evaluate the "then" branch. */
                        if (Token == TokenQuestionMark &&
                                Parser->Mode == RunModeRun &&
                                IS_NUMERIC_COERCIBLE(StackTop->Val)) {
                            int Cond = ExpressionCoerceInteger(StackTop->Val) != 0;
                            if (TernaryDepth < MAX_TERNARY_NESTING)
                                TernaryCondition[TernaryDepth] = Cond;
                            if (!Cond && IgnorePrecedence > Precedence) {
                                IgnorePrecedence = Precedence;
                                TernaryIgnoreDepth = TernaryDepth;
                            }
                        }
                        ...
                        case TokenColon:
                            TernaryDepth--;
                            /* leaving the "then" branch. If we were ignoring a
                                false condition's then-branch, stop ignoring.
                                If the condition was TRUE, start ignoring the
                                "else" branch now. */
                            if (Parser->Mode == RunModeRun) {
                                if (TernaryIgnoreDepth == TernaryDepth) {
                                    IgnorePrecedence = DEEP_PRECEDENCE;
                                    TernaryIgnoreDepth = -1;
                                }
                                if (TernaryDepth >= 0 &&
                                        TernaryDepth < MAX_TERNARY_NESTING &&
                                        TernaryCondition[TernaryDepth] &&
                                        IgnorePrecedence > Precedence) {
                                    IgnorePrecedence = Precedence;
                                    TernaryIgnoreDepth = TernaryDepth;
                                }
            ...
            if (LexGetToken(Parser, NULL, false) == TokenOpenBracket) {
             ...
             /* if we've successfully ignored the RHS turn ignoring off - but
                not while a ternary branch is being skipped, since that region
                is delimited by ':'/end-of-expression, not by precedence. */
            if (Precedence <= IgnorePrecedence && TernaryIgnoreDepth < 0)
                IgnorePrecedence = DEEP_PRECEDENCE;
    ...
    struct ExpressionStack **StackTop, const char *FuncName, int RunIt)
{
    ...
    if (RunIt) {
        ...
        if (FuncValue->Val->FuncDef.Intrinsic == NULL) {
            ...
            /* Support backward gotos: if we finished the body still searching
               for a label, it must be behind the goto, so re-scan the body
               from the top. Each re-scan that reaches a matching label flips
               the mode back to RunModeRun and continues execution from there.
               Bound the number of passes to avoid infinite loops on a truly
               missing label while still allowing loops built out of gotos. */
            {
                int GotoPasses = 0;
                const int MaxGotoPasses = 2000000;
                while (FuncParser.Mode == RunModeGoto &&
                        GotoPasses++ < MaxGotoPasses) {
                    const char *WantedLabel = FuncParser.SearchGotoLabel;
                    ParserCopy(&FuncParser, &FuncValue->Val->FuncDef.Body);
                    FuncParser.Mode = RunModeGoto;
                    FuncParser.SearchGotoLabel = WantedLabel;
                    if (ParseStatement(&FuncParser, true) != ParseResultOk)
                        ProgramFail(&FuncParser, "function body expected");
                }
            }
