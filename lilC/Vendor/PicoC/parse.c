...
#ifdef LILC_IOS_HOST
extern int LilCShouldStopExecution(void);
#define LILC_CHECK_RUN_LIMIT(parser) \
    do { \
        if ((parser)->Mode == RunModeRun && LilCShouldStopExecution()) \
            ProgramFail((parser), "program stopped: too many steps"); \
    } while (0)
#else
#define LILC_CHECK_RUN_LIMIT(parser) do { } while (0)
#endif
    ...
    struct ValueType *ReturnType, char *Identifier)
{
    ...
    if (Token == TokenSemicolon &&
            TableGet(&pc->GlobalTable, Identifier, &OldFuncValue, NULL, NULL, NULL)) {
        /* A prototype after a definition is valid C. Keep the existing function. */
...
/* parse a brace initializer for a struct or union and assign it positionally
   to the members in declaration order (which, for structs, is ascending byte
   offset). e.g. struct P p = {1, 2}; */
static int ParseStructInitializer(struct ParseState *Parser,
    struct Value *NewVariable, int DoAssignment)
{
    enum LexToken Token;
    struct Value *CValue;
    int MemberIndex = 0;
    int DoWork = (Parser->Mode == RunModeRun && DoAssignment);

    /* gather the members in declaration (offset) order */
    #define MAX_STRUCT_INIT_MEMBERS 256
    struct Value *OrderedMembers[MAX_STRUCT_INIT_MEMBERS];
    int MemberCount = 0;

    if (DoWork) {
        struct ValueType *StructType = NewVariable->Typ;
        struct Table *Members = StructType->Members;
        if (Members == NULL)
            ProgramFail(Parser, "can't initialize an incomplete type");

        for (short Bucket = 0; Bucket < Members->Size; Bucket++) {
            struct TableEntry *Entry;
            for (Entry = Members->HashTable[Bucket]; Entry != NULL;
                    Entry = Entry->Next) {
                if (MemberCount < MAX_STRUCT_INIT_MEMBERS)
                    OrderedMembers[MemberCount++] = Entry->p.v.Val;
            }
        }
        /* insertion sort by offset (Val->Integer) - member counts are small */
        for (int i = 1; i < MemberCount; i++) {
            struct Value *Key = OrderedMembers[i];
            int j = i - 1;
            while (j >= 0 &&
                    OrderedMembers[j]->Val->Integer > Key->Val->Integer) {
                OrderedMembers[j + 1] = OrderedMembers[j];
                j--;
            }
            OrderedMembers[j + 1] = Key;
        }
    }

    Token = LexGetToken(Parser, NULL, false);
    while (Token != TokenRightBrace) {
        struct Value *MemberTarget = NULL;

        if (DoWork) {
            if (MemberIndex >= MemberCount)
                ProgramFail(Parser, "too many struct initializers");
            /* union: all members share offset 0; struct: real offset */
            MemberTarget = VariableAllocValueFromExistingData(Parser,
                OrderedMembers[MemberIndex]->Typ,
                (union AnyValue*)(&NewVariable->Val->ArrayMem[0] +
                    OrderedMembers[MemberIndex]->Val->Integer),
                true, NewVariable);
        }
        ...
        if (DoWork) {
            ExpressionAssign(Parser, MemberTarget, CValue, false, NULL, 0, false);
            VariableStackPop(Parser, CValue);
            VariableStackPop(Parser, MemberTarget);
        }
    ...
    struct Value *NewVariable, int DoAssignment)
{
    ...
    if (LexGetToken(Parser, NULL, false) == TokenLeftBrace) {
        LexGetToken(Parser, NULL, true);
        if (NewVariable != NULL && (NewVariable->Typ->Base == TypeStruct ||
                NewVariable->Typ->Base == TypeUnion)) {
            /* struct/union brace initializer */
            ParseStructInitializer(Parser, NewVariable, DoAssignment);
        } else {
            /* array initializer */
